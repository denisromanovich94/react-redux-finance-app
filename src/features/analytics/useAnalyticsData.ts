...
export function useAnalyticsData() {
  ...
  const totals: Totals = useMemo(() => {
    ...
    for (const t of transactions) {
      if (t.amount > 0) {
        ...
        expenses += t.amount; // отрицательное
      }
    }
  ...
  const monthTotals: MonthTotals = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      if (!d.isValid() || !d.isSame(now, 'month')) continue;
      if (t.amount > 0) incomeM += t.amount;
      else if (t.amount < 0) expensesM += t.amount; // отрицательное
    }
